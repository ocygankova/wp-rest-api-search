...
(() => {
/*!****************************************!*\
  !*** ./src/wp-rest-api-search/view.js ***!
  \****************************************/
__webpack_require__.r(__webpack_exports__);
...
async function fetchPosts(query, postTypes, signal) {
  ...
  const requests = postTypes.map(type => fetch(`/wp-json/wp/v2/${type}?search=${cleanValue}&_embed`, {
    signal
  }));
...
function generateHTML(data) {
  ...
  data.forEach(item => {
    ...
    if (item._embedded && item._embedded['wp:featuredmedia'] && item._embedded['wp:featuredmedia'][0]) {
      ...
      const imgUrl = img.media_details?.sizes?.thumbnail?.source_url || img.source_url;
      if (imgUrl) {
